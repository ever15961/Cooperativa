<?php
    ...
    define("SELECCIONAR_PRESTAMO_PARAMETRO", "SELECT p.id, p.montoPrestamo, i.tasaInteres, p.plazo_anio, p.plazo_cuota FROM prestamo p
    INNER JOIN interes i
    ON i.id=p.destinoPrestamo
    WHERE p.id = ?");

    define("SQL_PRUEBA", "SELECT p.id, p.montoPrestamo, i.tasaInteres, p.plazo_anio, p.plazo_cuota, i.destino, s.nombre, s.apellido, s.direccion FROM prestamo p
    ...
    i.tasaInteres, p.saldo_pendiente FROM socio s
    INNER JOIN prestamo p 
    ON s.id=p.socio
    INNER JOIN interes i
    ON i.id=p.destinoPrestamo
     WHERE p.estadoPrestamo = 1 AND s.codigoSocio = ?");

    define("SELECCIONAR_SOCIO_PRESTAMO", "SELECT s.codigoSocio, s.nombre, s.apellido, s.direccion FROM socio s WHERE s.codigoSocio = ?");
 
    define("SELECCIONAR_PRESTAMOS_REPORTE", "SELECT p.id, p.plazo_anio, p.plazo_cuota, i.destino, p.fechaInicio, p.estadoPrestamo, p.montoPrestamo, i.tasaInteres
    FROM prestamo p 
    INNER JOIN 
